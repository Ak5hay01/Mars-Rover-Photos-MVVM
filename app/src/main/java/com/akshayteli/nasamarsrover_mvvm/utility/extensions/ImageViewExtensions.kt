package com.akshayteli.nasamarsrover_mvvm.utility.extensions
...
import com.akshayteli.nasamarsrover_mvvm.utility.view.GlideApp
import com.akshayteli.nasamarsrover_mvvm.utility.view.PhotosRequestListener
import com.akshayteli.nasamarsrover_mvvm.utility.view.PhotosAppGlideModule
...
fun ImageView.setImageFromUrlWithProgressBar(url: String, progress: View) {
    GlideApp.with(this.context).load(url.toUri().buildUpon().scheme("https").build()).thumbnail(0.1f).listener(
        PhotosRequestListener(progress)
    )
        .into(this)
}
